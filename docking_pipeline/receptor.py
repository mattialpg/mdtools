...
def prepare_receptor(ctx, archive_dir=ARCHIVE_DIR):
    """Prepare receptor PDB/PDBQT using archive-or-reconstruct logic."""
    workdir = Path(ctx.workdir)
    receptor_id = ctx.receptor_id
    ...
    run_in_terminal([ctx.obabel, str(pdb_outfile), "-xr", "-O", str(receptor_pdbqt)])
    ctx.receptor_pdbqt = receptor_pdbqt
    return ctx
